            ...
            <div class="col-3 products">
                <a href="/productDetails/{{$product->id}}" style="color: black">
                    ...
                    <div class="col-12" style="text-align: center">
                        <p>{{$product->name}}</p>
                        <p class="showPriceInBlock"> {{number_format($product->price)}} تومان</p>
                    </div>
                </a>
            </div>
            ...
            <div class="col-9" style="text-align: center;padding: 20px;margin-bottom: 15px">
                @for($index=$pages;$index>0;$index--)
                    @if($index==$currentPage)
                        <a href="/brands/{{$brand_id}}?page={{$index}}" style="border: 1px solid black;padding: 6px 12px;border-radius: 5px;background-color: #8f8f8f;color: black;text-decoration: none">{{$index}}</a>
                    @else
                        <a href="/brands/{{$brand_id}}?page={{$index}}" style="border: 1px solid black;padding: 6px 12px;border-radius: 5px;color: black;text-decoration: none">{{$index}}</a>
                    @endif
                @endfor
            </div>
        </div>
